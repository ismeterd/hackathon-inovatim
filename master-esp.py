...
class Broker:
    ...
    def __process(self, addr: tuple, raw_data: bytes):
        ...
        if client_type.lower() == "pub":  # If the client is publisher
            ...
            self.__log("[INFO]: Data is published to the topic \'{}\'".format(topic_name))

        else:  # If the client is subscriber
            self.subscribers.update({addr: topic_name})
            self.__log("\'[INFO]: {}:{}\' is subscribed to the topic \'{}\'".format(
                    ...
                    for client_addr, topic_name in self.subscribers.items():
                        if topic_name == topic_name_search:
                            try:
                                ...
                                self.__log("[INFO]: Subscriber in {}:{} is dead, removed from the subscriber map".format(
                                    client_addr[0], client_addr[1]))

                    # Delete data if it is sent to a subscriber(s)
                    if send_to_n_subs != 0:
                        self.topics.update({topic_name_search: None})
                        self.__log("[INFO]: Subscribers of the topic \'{}\' are notified".format(topic_name_search))
                        send_to_n_subs = 0
    ...
    def run(self):
        # Initialize socket
        self.__log("[INFO]: Master ESP is started.")
